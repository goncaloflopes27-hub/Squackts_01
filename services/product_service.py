...
class ProductService:
    ...
    def list_all(self, query: str = "", tipo: str = "todos"):
        rows = products_repo.list_all(self.conn)
        filtered = []
        q = query.lower().strip()
        for row in rows:
            if tipo != "todos" and row["tipo_producao"] != tipo:
                continue
            if q and q not in row["nome"].lower() and q not in row["sku"].lower():
                continue
            filtered.append(row)
        return filtered

    def get_by_id(self, product_id: str):
        return products_repo.get_by_id(self.conn, product_id)


    def create_product(self, **data) -> str:
        return self.save(data)

    def update_product(self, product_id: str, **data) -> str:
        data["id"] = product_id
        return self.save(data)

    def list_products(self, filters: dict | None = None):
        filters = filters or {}
        return self.list_all(query=filters.get("query", ""), tipo=filters.get("tipo", "todos"))
